...
const faqs = [
  ...
  ["Does CamSentinel require new cameras?", "No. CamSentinel is designed to work with common IP camera and NVR environments, subject to stream access and network compatibility."],
  ["Can this demo connect to a live camera?", "No. This public product demo is intentionally frontend-only and uses synthetic imagery and simulated operational data."],
      ...
      <main>
        <section className="hero">
          <div className="container hero-grid">
            <div className="hero-copy">
              ...
              <div className="hero-actions"><Link className="btn btn-primary" href="/signup">Start free trial <ArrowRight size={17} /></Link><Link className="btn btn-secondary" href="/app/overview">Explore live demo</Link></div>
              <div className="trust-row"><span className="trust-item"><CheckCircle size={16} weight="fill" color="var(--emerald)" /> Works with existing IP cameras</span><span className="trust-item"><CheckCircle size={16} weight="fill" color="var(--emerald)" /> No backend exposed</span><span className="trust-item"><CheckCircle size={16} weight="fill" color="var(--emerald)" /> Setup in minutes</span></div>
            </div>
        ...
        <section className="section" style={{ background: "var(--paper)", borderBlock: "1px solid var(--line)" }}>
          <div className="container split">
            <div className="media-frame"><Image src="/images/receiving-dock-camera.png" alt="Synthetic CCTV view of a warehouse receiving dock" width={1536} height={1024} /></div>
        ...
        <section className="section" id="faq"><div className="container split" style={{ alignItems: "start" }}><div><span className="eyebrow">Common questions</span><h2 className="section-title">Clear answers before you connect a camera.</h2><p className="lede">This public build demonstrates the complete customer journey using simulated data.</p></div><div className="metric-list">{faqs.map(([q,a]) => <details key={q} className="metric-row" style={{ display: "block" }}><summary style={{ fontWeight: 650, cursor: "pointer" }}>{q}</summary><p className="muted" style={{ lineHeight: 1.6 }}>{a}</p></details>)}</div></div></section>

        <section className="section-tight"><div className="container"><div className="cta-panel"><div><span className="eyebrow" style={{ color: "#7fd6b8" }}>Start with the cameras you already own</span><h2 className="section-title" style={{ marginBottom: 12 }}>Build a clearer picture of every site.</h2><p className="muted">Explore the complete mock SaaS experience or start the onboarding flow.</p></div><Link className="btn btn-emerald" href="/signup">Start free trial <ArrowRight size={17} /></Link></div></div></section>
      </main>
      <SiteFooter />
    </>
  );
