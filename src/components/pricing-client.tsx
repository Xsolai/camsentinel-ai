...
export function PricingClient() {
  ...
  return (
    <>
      ...
      <div className="estimator"><div><strong>Estimate your camera rollout</strong><p className="muted" style={{ margin: "8px 0 0" }}>This only adjusts the simulated checkout summary.</p></div><div style={{ minWidth: 260 }}><label htmlFor="camera-count" style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}><span>Cameras</span><strong>{cameras}</strong></label><input id="camera-count" type="range" min="4" max="100" step="4" value={cameras} onChange={(event) => setCameras(Number(event.target.value))} style={{ width: "100%", accentColor: "var(--ink)" }} /></div></div>
    </>
  );
}
